...
class HistogramDisplay: UIView, HistogramGraph {
    ...
    func histogramCalculation(imageRef: CGImage) -> (red: [UInt], green: [UInt], blue: [UInt]) {
        ...
        var alpha: [UInt]  = Array(repeating: 0, count: 256)
        ...
        let alphaPtr = alpha.withUnsafeMutableBufferPointer { $0.baseAddress }
        ...
        let _:vImage_Error = vImageHistogramCalculation_ARGB8888(&inBuffer, histogram, UInt32(kvImageNoFlags))
    ...
    let redLayer = HistogramDisplay.createLayerForColor(color: .black)
    let greenLayer = HistogramDisplay.createLayerForColor(color: .black)
    let blueLayer = HistogramDisplay.createLayerForColor(color: .black)
    ...
    static func createLayerForColor(color: UIColor) -> CAShapeLayer {
        ...
        layer.fillColor = color.withAlphaComponent(1).cgColor
